...
class PressToTalk:
    ...
    def update_profile_summary(self, user_text: str, ai_response: str):
        """
        대화 내용을 바탕으로 기존 프로필 요약을 업데이트합니다.
        (Gemini API를 추가로 호출합니다)
        """
        if not self.current_user_name:
            return
        
        print(f"⏳ {self.current_user_name}님의 프로필 요약 업데이트 시도...")
