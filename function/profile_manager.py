...
class ProfileManager:
    ...
    def load_profile_for_chat(self, name: str):
        ...
        # 2. 💡 강화된 시스템 명령어 생성 (Gemini가 '강은성'님임을 인지하도록 강제)
        ...
        #    (이것이 로봇이 '내가 강은성님과 대화 중'임을 확실히 인지하게 합니다.)
    ...
    def update_summary_after_chat(self, user_text: str, ai_response: str):
        ...
        try:
            ...
            # 5. self.chat 객체를 새 요약 정보로 재-초기화 (history 보존)
            enhanced_system_instruction = (
                SYSTEM_INSTRUCTION +
                f"\n\n--- 현재 시간 ---\n"
                f"오늘은 {current_date_str_for_chat}입니다."
                "\n\n--- 중요 기억 (필독!) ---\n"
                f"당신은 지금 **'{self.ptt.current_user_name}'님**과 대화하고 있습니다. **사용자의 이름을 잊지 말고 항상 기억하세요.**\n" # <--- 이름 강조
            ...
            self.ptt.chat = genai.GenerativeModel(
                self.MODEL_NAME, 
                system_instruction=enhanced_system_instruction
            ).start_chat(history=current_history) # <--- 기존 대화 기록을 유지하며 시스템 프롬프트만 갱신
